...
def emit_shapes_fill(out: Path, db: dict, variants: list) -> None:
    ...
    L.append("}")
    L.append("")
    L.append("// Resolve every named operand field of a variant at ONCE with a")
    L.append("// SINGLE vi switch (no per-position dispatch, no loop).  `out`")
    L.append("// receives the field pointers in role order; sized for the")
    L.append("// maximum operand count (9).  Used by the interpreter's")
    L.append("// OperandFields view and by operand_field below.")
    L.append("inline void fill_operand_fields(std::uint32_t vi,")
    L.append("                                 const DecodedInstruction* inst,")
    L.append("                                 const OperandValue* out[16]) {")
    L.append("    switch (vi) {")
    for vi, _v in enumerate(variants):
        g2 = vid2gid[vi]
        type_name = shape_type_name(order[g2])
        fields = group_field_names(groups[order[g2]])
        if fields:
            L.append(f"    case {vi}: {{")
            L.append(f"        const auto& o = *static_cast<const {type_name}*>(inst);")
            for fp, f in enumerate(fields):
                L.append(f"        out[{fp}] = &o.{f};")
            L.append("        break;")
            L.append("    }")
        else:
            L.append(f"    case {vi}: break;  // no operands")
    L.append("    default: break;")
    L.append("    }")
    L.append("}")
    L.append("")
    L.append("// Pointer to one named operand FIELD of a typed decoded shape.")
    L.append("// Thunks through fill_operand_fields: single vi dispatch, no")
    L.append("// per-position switch.  Returns nullptr when p >= n_ops.  This is")
    L.append("// the decode/CLI/test bridge; the interpreter reads fields via the")
    L.append("// OperandFields view (single fill call).")
    L.append("inline const OperandValue* operand_field(std::uint32_t vi,")
    L.append("                                         const DecodedInstruction* inst,")
    L.append("                                         std::uint16_t p) {")
    L.append("    if (!inst || p >= inst->n_ops) return nullptr;")
    L.append("    const OperandValue* f[16];")
    L.append("    fill_operand_fields(vi, inst, f);")
    L.append("    return f[p];")
    L.append("}")
    L.append("")
    L.append("}  // namespace semu::shape")
