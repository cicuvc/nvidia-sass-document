#!/usr/bin/env python3
"""Extract the sm_120 (Blackwell consumer) SASS ISA description into a queryable JSON DB.

Reads:  sm120_instructions.txt, sm120_latencies.txt
Writes: sm120.json  (default: repo root)
...
and every CLASS / ALTERNATE CLASS encoding variant.
...
INSTR = REPO / "sm120_instructions.txt"
LAT = REPO / "sm120_latencies.txt"

# The base functional-unit pipes (latency file OPERATION SETS).
# sm100/sm120 add ttu_pipe (tree-traversal / ray-tracing unit).
BASE_PIPES = [
    "int_pipe", "mio_pipe", "fe_pipe", "fmalighter_pipe", "fp16_pipe",
    "cbu_pipe", "fma64lite_pipe", "fma64heavy_pipe", "udp_pipe", "ttu_pipe",
]
...
def main():
    ap = argparse.ArgumentParser(description="Extract sm120 SASS ISA -> JSON")
    ap.add_argument("-o", "--out", default=str(REPO / "sm120.json"))
    ...
    db["meta"] = {
        ...
        "encoding_width": 128,
        "note": "Header says WORD_SIZE 64 / Volta, but sm_120 instructions are 128-bit.",
    }
