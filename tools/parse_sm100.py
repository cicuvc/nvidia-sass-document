...
INSTR = REPO / "sm100_instructions.txt"
LAT = REPO / "sm100_latencies.txt"

# The base functional-unit pipes (latency file OPERATION SETS).
# sm100 adds ttu_pipe (tree-traversal / ray-tracing unit).
BASE_PIPES = [
...
def _parse_enum_body(body: str) -> dict:
    """Parse quoted/bare enum names, implicit values, and range entries."""
    vals: dict = {}
    implicit = None
...
def validate(db):
    ...
    # sm100 counts differ from sm90; the structural invariants below still hold.
    for x in v:
        if x.get("remap"):
