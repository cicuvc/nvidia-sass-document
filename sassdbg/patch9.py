"""sassdbg M9 — runtime breakpoints with ZERO register reservation.

Drop-in replacement (in development) for patch.py's M3v3 Debugger.
...
Probe-phase lessons baked in (see AGENTS.md M9 section):
  ...
  assert every code image fits its slot.
"""
...
class Layout:
    ...
    def __init__(self, max_bps: int, max_warps: int):
        ...
        self.cmdseq = self.hslots + max_warps * 32 * 16   # mw x 16B
...
def _handler_src(lay: Layout, arena: int, warp: int,
    ...
    return f"""\
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_R2 + 8:x}], R4;[7:7:{{}}:8:0]
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_R2 + 0xC:x}], R5;[7:7:{{}}:8:0]
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_R2 + 0x10:x}], R6;[7:7:{{}}:8:0]
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_R2 + 0x14:x}], R7;[7:7:{{}}:8:0]
    P2R R2, PR;[2:7:{{}}:6:0]
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_PR:x}], R2;[7:7:{{2}}:8:0]
    LDG.E.STRONG.GPU R7, [{{R0,R1}}+0x{F_RELEASE:x}];[2:7:{{}}:8:0]
    LDG.E.64.STRONG.GPU {{R2,R3}}, [{{R0,R1}}+0x{F_SITE:x}];[3:7:{{}}:8:0]
    MOV32I R4, 0x{pool & 0xFFFFFFFF:08x};[7:7:{{}}:5:1]
    IADD3 R4, R0, -R4, RZ;[7:7:{{}}:5:1]
    SHF.R.U32.HI R4, RZ, 0x7, R4;[7:7:{{}}:5:1]
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_F:x}], R4;[7:7:{{}}:8:0]
    BMOV R5, MACTIVE;[4:7:{{}}:8:0]
    FLO.U32 R6, PT, R5;[5:7:{{4}}:5:1]
    LOP3.LUT R5, R4, 0x1F, RZ, 0xC0;[7:7:{{}}:5:1]
    ISETP.EQ.AND P6, PT, R5, R6, PT;[7:7:{{5}}:13:1]
    IADD3 R5, R4, -R5, RZ;[7:7:{{}}:5:1]
    IADD3 R5, R5, R6, RZ;[7:7:{{}}:5:1]
    MOV32I R4, 0x{hslots & 0xFFFFFFFF:08x};[7:7:{{}}:5:1]
    IMAD R4, R5, 0x10, R4;[7:7:{{}}:5:1]
    MOV32I R5, 0x{hslots >> 32:08x};[7:7:{{}}:5:1]
    @P6 LDG.E.STRONG.GPU R6, [{{R4,R5}}+0xC];[4:7:{{}}:8:0]
    @P6 IADD3 R6, R6, 0x1, RZ;[7:7:{{4}}:5:1]
    @P6 STG.E.STRONG.GPU [{{R4,R5}}+0x4], R2;[7:7:{{3}}:8:0]
    @P6 STG.E.STRONG.GPU [{{R4,R5}}+0x8], R3;[7:7:{{3}}:8:0]
    @P6 BMOV R2, MACTIVE;[4:7:{{}}:8:0]
    @P6 STG.E.STRONG.GPU [{{R4,R5}}], R2;[7:7:{{4}}:8:0]
    @P6 STG.E.STRONG.GPU [{{R4,R5}}+0xC], R6;[7:7:{{}}:8:0]
#def_label(dbgspin)
    ...
    STG.E.STRONG.GPU [{{R0,R1}}+0x{F_CMD:x}], R3;[7:7:{{3}}:8:0]
    CCTL.I.IVALL;[7:7:{{}}:4:0]
...
""" + _ret_surgery(t00, t10, t11) + f"""\
    ...
    STG.E.128.STRONG.GPU [{{R2,R3}}], {{R4,R5,R6,R7}};[7:7:{{}}:8:0]
    CCTL.I.IVALL;[7:7:{{}}:4:0]
""" + "    NOP;[7:7:{}:8:0]\n" * 32 + f"""    CCTL.I.IVALL;[7:7:{{}}:4:0]
...
def _handler_anchors(src: str) -> tuple[int, int]:
    ...
    final RET (self-overwritten); cmdret = the BRA right after the
    command CALL (re-enters the spin)."""
    n = 0
    cmdret = None
    prev_call = False
    for ln in src.splitlines():
        ...
        if ln.startswith("#def_label"):
            continue
        if prev_call and ln.startswith("BRA #label(dbgspin)"):
            cmdret = n * 16
...
class Debugger:
    ...
    def _jmp_word(self, va: int) -> tuple[int, int]:
        enc = assemble_flat(f"    JMP 0x{va:x};[7:7:{{}}:6:0]\n")
        assert len(enc) == 1
        return enc[0]
    ...
    MOV32I R4, 0x{cmdseq & 0xFFFFFFFF:08x};[7:7:{{}}:5:1]
    MOV32I R5, 0x{cmdseq >> 32:08x};[7:7:{{}}:5:1]
    STG.E.STRONG.GPU [{{R4,R5}}+0x4], R2;[7:7:{{2}}:8:0]
    MOV32I R2, 0x{hdl & 0xFFFFFFFF:08x};[7:7:{{}}:5:1]
    MOV32I R3, 0x{hdl >> 32:08x};[7:7:{{}}:5:1]
        ...
        for i, r in enumerate(regs):
            ...
            if ru in self._FRAME_SLOT:
                insts += [
                    ...
                    f"{self._FRAME_SLOT[ru]:x}];[2:7:{{}}:8:0]",
    ...
    def set_reg(self, warp: int, reg: str, val: int, lane: int = 0) -> None:
        ...
        if r in self._FRAME_SLOT:
            insts = pre + [
                ...
                f"{self._FRAME_SLOT[r]:x}], R3;[7:7:{{}}:8:0]",
            ]
