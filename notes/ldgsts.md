...
(`LDGDEPBAR` + `DEPBAR.LE`, see `notes/depbar.md`), **not** an mbarrier.
...
`desc[UR]` global-addressing convention seen in `LDG`/`STG` (`notes/ldg.md`).
...
`notes/depbar.md`. `IDEST_SIZE=0` (no register result — data lands in shared).
