...
Warpgroup-level async tensor core operation for binary data. Uses the same GMMA architecture as HGMMA — see `notes/wgmma.md` and `notes/hgmma.md` for sync model, accumulator collector, and GMMA scoreboard details.
...
Same GMMA layout as HGMMA (`notes/hgmma.md`), with opcode and size/type fields adapted for binary data:
...
## Latency
`mio_pipe`, async GMMA scoreboard. See `notes/wgmma.md` for GMMA completion model.
