...
2. **Small fixed per-instruction K (16 for fp16, 32 for fp8).** The reduction
   ...
   `wgmma.md`). That matches "compute a depth-16 dot product, add to
   accumulator" (FMA tree), not a deep systolic reduction pipeline.
3. **Per-subcore 16-row partitioning.** `wgmma.md`: each of the 4 subcores
   has its own tensor unit computing 16 of the 64 rows — a widened per-subcore
   Ampere unit, not one SM-wide array.
4. **implicit-collector → explicit-TMEM world-jump to Blackwell.** `wgmma.md`
   (Hopper keeps the accumulator inside the tensor core, register-named,
   drain-on-read) vs `tcgen05_vs_wgmma.md` (Blackwell externalises it to
...
- The internal-collector behaviour (`wgmma.md`) is **equally consistent**
