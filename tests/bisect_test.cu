// Bisect: morph variants(fast) -> fill_test(slow) inside ONE module.
#include <cuda.h>
#include <cstdint>
...
// (a) exact clone of variants' fast kernel
extern "C" __global__ void __launch_bounds__(128)
chain_base(float* out) {
...
// (b) + 2 unused params
extern "C" __global__ void __launch_bounds__(128)
chain_p3(float* out, uint32_t fill, uint32_t do_fill) {
    __shared__ __align__(1024) uint8_t smem[16384];
    CHAIN64
}

// (c) + dead fill code + BAR (fill_test clone)
extern "C" __global__ void __launch_bounds__(128)
chain_deadfill(float* out, uint32_t fill, uint32_t do_fill) {
    __shared__ __align__(1024) uint8_t smem[16384];
    if (do_fill) {
...
// (d) dead fill code, no BAR
extern "C" __global__ void __launch_bounds__(128)
chain_deadfill_nobar(float* out, uint32_t fill, uint32_t do_fill) {
    __shared__ __align__(1024) uint8_t smem[16384];
    if (do_fill) {
        uint32_t* s = (uint32_t*)smem;
        for (int i = threadIdx.x; i < 16384 / 4; i += 128) s[i] = fill;
