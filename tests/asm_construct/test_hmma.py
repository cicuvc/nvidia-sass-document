...
KERNEL = """#fn k(out<2048>) {
    ...
    NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]
    HMMA.16816.F32.BF16 {R28,R29,R30,R31}, {R16,R17,R18,R19}, {R20,R21}, {R24,R25,R26,R27};[7:7:{5}:1:0]
    NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]
    NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]  NOP;[7:7:{}:5:1]
...
# --- 1. A=B=1.0, C=10..13 -> D = 1*8 + C = 18,19,20,21 (nvcc mma_frag2) ---
check("HMMA A=1,B=1,C=10..13 -> 18,19,20,21",
      run_hmma(frag(0x3F800000, 0x3F800000, [10, 11, 12, 13])),
      [18.0, 19.0, 20.0, 21.0])

# --- 2. A=2.0 (bf16 2.0 = 0x4000) -> D = 2*8 + C = 26..29 ------------------
check("HMMA A=2,B=1 -> 26,27,28,29",
      run_hmma(frag(0x40000000, 0x3F800000, [10, 11, 12, 13])),
      [26.0, 27.0, 28.0, 29.0])

# --- 3. A=0 -> D = C = 10..13 (no A contribution) ---------------------------
check("HMMA A=0 -> C=10,11,12,13",
      run_hmma(frag(0, 0x3F800000, [10, 11, 12, 13])),
      [10.0, 11.0, 12.0, 13.0])

# --- 4. encoding vs nvcc (data bits, same registers R4,R4,R2,R12) ----------
e = assemble_flat(
...
check("HMMA encode matches nvcc (bits 0-104)", diff, [])

# --- 5. all stall/yield schedules accepted & run (verified on hardware) -----
...
print(f"\n=== HMMA (tensor core bf16a32): {'ALL PASS' if ok else 'FAILURES'} ===")
