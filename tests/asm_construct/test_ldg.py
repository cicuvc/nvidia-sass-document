...
#
# LDG is variable-latency AND depends on the `desc[UR4]` cache-policy
...
#   LDCU.64 UR4, c[0x0][0x358];[0:7:{}:1:0]   # wr=0 -> sets SB0
#   LDG.E  R20, desc[UR4][R6.64+0x20];[1:7:{0}:5:1]  # req={0} waits SB0
...
def build():
    lines = ["#fn ldg_test(out<1024>) {",
             "    LDCU.64 UR4, c[0x0][0x358];[0:7:{}:1:0]",   # desc policy, wr=SB0
             "    LDC.64 R6, #param(out);[0:7:{}:1:0]",
             "    LDG.E R20, desc[UR4][R6.64+0x20];[1:7:{0}:5:1]",  # wait SB0, wr=SB1
...
print("before using desc[UR4]; plus a result scoreboard (wr) consumed by the")
