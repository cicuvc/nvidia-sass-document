...
# ---------------------------------------------------------------------------
# int_pipe -> mio_pipe (LDG address/AGU) forwarding calibration (SM120)
#
# Same stale/fresh boundary method, with the mio consumer being an LDG whose
# ADDRESS register pair is produced by an int op:
#   poison : LDC.64 {R10,R11} = addrA   (wr=SB0; buffer A holds marker MA)
#   producer (int_pipe): MOV.64 {R10,R11} = {R12,R13}   (addrB, req={0})
#   consumer (mio_pipe): LDG.E R20, desc[..][{R10,R11}+0x0]
# If the int MOV write has not landed when the AGU reads the address pair,
# the LDG loads from buffer A -> R20 = MA (stale); once landed -> MB (fresh).
#
# Spec sm120 TABLE_TRUE(GPR): FXU_OPS -> MIO_FAST_OPS = 6.
# See notes/sm90/arch/pipe_forwarding.md.
# ---------------------------------------------------------------------------

MA = 0x11111111                # marker in buffer A (poison address)
MB = 0x22222222                # marker in buffer B (true address)
STALLS = [0] + list(range(1, 17)) + [30]
SWEEP = slice(1, 1 + 16)
SPEC_L = 6                     # FXU_OPS -> MIO_FAST_OPS
...
def build_kernel(stalls):
    lines = ["#fn fwd(out<8>, addrA<8>, addrB<8>) {",
             ...
             "    LDC.64 {R12, R13}, #param(addrB);[3:7:{}:2:0]"]
    for i, S in enumerate(stalls):
        ...
        if S == 0:                      # poison-only control (no producer)
            ...
            lines.append("    MOV.64 {R10, R11}, {R12, R13};[7:7:{0}:1:1]")
            for _ in range(S - 1):
...
print("int->mio (LDG address/AGU) forwarding calibration (SM120); "
      "buffer A marker=0x%08X, buffer B marker=0x%08X" % (MA, MB))
...
for j in range(len(sweep)):
    if all(c == "F" for c in sweep[j:]):
        minG = STALLS[SWEEP][j]
        break
print(f"MOV.64->LDG  Lspec={SPEC_L} [{'det' if det else 'var'}]: {sweep}")
if minG is not None:
...
ok = True
if maj[0] != "S":                       # poison-only must load from buffer A
    ok = False
    print("        FAIL: poison-only control should read stale (marker A)")
if maj[-1] != "F":                      # huge-gap must load from buffer B
    ok = False
...
print("\n=== int->mio forwarding calibration: "
